...
| `service/BrokerPortfolioSyncService` | Сверяет локальное состояние с портфелем брокера | `restorePositions()` восстанавливает после запуска только позиции с положительным количеством; внешний шорт логируется и не попадает в состояние бота. `synchronizeLongPositionForSell()` запрещает случайное открытие шорта |
