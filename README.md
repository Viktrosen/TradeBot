...
`thinkingConfig` не передаётся: используются настройки рассуждений выбранной модели
...
Для диагностики ошибок Gemini на уровне `DEBUG` фильтр пишет `requestId`, инструмент,
стратегию, модель, путь endpoint и фактически отправленный `generationConfig` (включая
JSON-схему). Ответ связывается с запросом по `requestId`: логируются
HTTP-статус и размер тела, а при ошибке — код/статус провайдера, типы деталей,
`reason`, `domain` и имена полей из `fieldViolations`, если Gemini их вернул.
