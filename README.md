...
2. Для открытой позиции сначала безусловно проверяются стоп-лосс и тейк-профит по текущей цене.
3. Если риск-выход не сработал, активная стратегия возвращает `BUY`, `SELL` либо `HOLD`. Для свечной стратегии сигнал рассчитывается только по новой закрытой свече.
4. В long-only режиме `SELL` без открытой long-позиции игнорируется; при наличии позиции это сигнал на её закрытие, а не открытие шорта.
5. Стратегические `BUY` и `SELL` подтверждаются `AiTradeSignalFilter`, если включён `AI_ENABLED`. При выключенном AI запрос к OpenRouter не отправляется, и бот следует стратегии.
6. `TradeDecisionService` открывает только новую long-позицию, а `TradingBotService` передаёт одобренное стратегическое закрытие в `PositionLifecycleService`.
7. `PositionLifecycleService` создаёт заявку, ждёт исполнение, сохраняет `TradeEvent` и публикует событие.
