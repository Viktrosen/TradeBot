...
`MarketDataProvider` запрашивает M5-историю в безопасном окне шесть дней для
индикаторов и режима рынка. `CandlestickPatternStrategy` использует выбранный
пользователем таймфрейм отдельно, но также ограничивает период одного запроса
по лимитам T-Invest API. Это предотвращает ошибку `30014` при запросе свечей.
