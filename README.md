...
в формате JSON-объекта с полями `action`, `confidence` и `reason`. Для закреплённой
...
лог HTTP-статус, модель и безопасно усечённый ответ. Защитные, ручные и аварийные
