...
3. Для открытой позиции сначала безусловно проверяются фиксированный стоп-лосс, защита накопленной прибыли и тейк-профит по текущей цене.
...
8. После открытия LONG или SHORT `PositionLifecycleService` создаёт у брокера только стоп-лосс. Тейк-профит и защита накопленной прибыли контролируются ботом по текущей цене; идентификатор и цена брокерского SL, а также состояние защиты прибыли сохраняются в PostgreSQL.
...
Закрытия по стоп-лоссу, тейк-профиту, ручной команде и аварийному закрытию не проходят через AI, лимиты размера позиции или фильтры стратегии. Перед самостоятельным закрытием бот отменяет активный брокерский стоп-лосс, **повторно подтверждает у брокера, что он не исполняется**, и только затем использует рыночную заявку с повторными попытками. Если отмена или конечный статус не подтверждены, встречная заявка не отправляется: позиция остаётся под брокерской защитой до сверки.

### AI-подтверждение сигналов

`AiTradeSignalFilter` передаёт выбранному через `AI_PROVIDER` провайдеру (Gemini или GigaChat)
выбранную стратегию и её объяснение,
...
Для Gemini ответ запрашивается
...
`confidence` — диапазоном от 0 до 1; бот дополнительно валидирует ответ.
`thinkingConfig` не передаётся: используются настройки рассуждений выбранной Gemini-модели
(для `gemini-3.5-flash-lite` — `minimal`). `GEMINI_THINKING_BUDGET` больше не используется.
В запросе задана `temperature: 1.0` согласно
[рекомендации Google для Gemini 3](https://ai.google.dev/gemini-api/docs/generate-content/gemini-3).
За JSON отвечают `responseMimeType` и `responseJsonSchema`. Парсер исключает части
с `thought: true` и объединяет итоговые текстовые части первого кандидата.
При `finishReason`, отличном от `STOP`, включая `MAX_TOKENS`, решение отклоняется;
при отсутствии этого поля итоговый JSON всё равно проходит обычную валидацию.

GigaChat использует OAuth-токен, который бот получает по ключу авторизации и обновляет
до истечения. Его ответ также проходит локальную строгую проверку полей `action`,
`confidence` и `reason`; поэтому текст вне JSON, неполный JSON или нестандартный формат
не могут одобрить сделку. Для персонального scope запросы к GigaChat сериализуются:
одновременно выполняется не более одной проверки, конкурирующий сигнал отклоняется без
ожидания, чтобы не торговать устаревшей свечой.

Запрос JSON не является заменой проверки на стороне бота:
если выбранный провайдер вернул ошибку, таймаут, неполный или невалидный JSON, AI-фильтр
консервативно отклоняет только соответствующую стратегическую сделку и пишет в
лог HTTP-статус, модель, `finishReason` и безопасно усечённый итоговый ответ без частей reasoning. Защитные, ручные и аварийные
закрытия AI-фильтр не блокирует.

Для диагностики ошибок на уровне `DEBUG` фильтр пишет `requestId`, инструмент,
стратегию, модель, путь endpoint и безопасные параметры генерации. Для Gemini это также
включает фактически отправленный `generationConfig` с JSON-схемой; для GigaChat —
`temperature`, лимит токенов и признак нестримингового ответа. Ответ связывается с
запросом по `requestId`: логируются HTTP-статус и размер тела, а для Gemini при ошибке —
код/статус провайдера, типы деталей, `reason`, `domain` и имена полей из
`fieldViolations`, если они возвращены.
Промпт, заголовки, ключ и полное тело ответа в эти DEBUG-логи не попадают.
Общий `INVALID_ARGUMENT` без деталей по-прежнему не позволяет определить ошибочный параметр.
...
| `service/BotOperationJournal` | Асинхронный журнал важных операций | Одновременно пишет решения и сбои AI, торговые решения, подтверждённые исполнения заявок, защитные события и сбои планировщика в обычный лог и `bot_operation_logs`; пишет пачками, не останавливает торговый цикл при сбое БД и не принимает тики/DEBUG-диагностику |
...
- **`SuperTrendStrategy`** — детерминированный ATR-based trend following по закрытым M5-свечам; временно отключён для автоматических новых входов до отдельной replay-валидации;
...
| `WEAK_TREND` | Новый вход временно не создаётся: SuperTrend проходит replay-валидацию |
...
У уже открытой позиции сохраняется идентификатор стратегии входа в `OPEN`-событии. Поэтому последующая смена режима влияет только на новые входы; стратегические выходы продолжают анализироваться той стратегией, с которой позиция была открыта. Исключение — ранее открытые SuperTrend-позиции: пока стратегия валидируется, они сопровождаются только безусловными риск-выходами. Стоп-лосс, тейк-профит, ручное и аварийное закрытие не зависят от режима рынка.

### Аудит исполненных сделок

Новые `OPEN`-события в `trade_events` дополнительно содержат `entry_strategy_id`,
`entry_market_regime`, ключ закрытой сигнальной свечи, уверенность и ограниченный
JSON-снимок индикаторов. Для новых `CLOSE`-событий записываются максимальное
благоприятное и неблагоприятное движение (`mfe_percent`, `mae_percent`) в процентах
от цены входа. Это не тиковый журнал: значения фиксируются только при закрытии.

После перезапуска путь цены уже открытой позиции неизвестен, поэтому восстановленная
позиция получает `excursion_complete=false` и MFE/MAE для неё намеренно остаются
`null`. Старые события также остаются полностью совместимыми: новые поля nullable.
...
- `stopLossPercent`, `takeProfitPercent` — исходная граница убытка и цель прибыли для открытой позиции; они же задают момент запуска динамической защиты прибыли;
...
В production-режиме после каждого открытия LONG или SHORT бот создаёт у брокера одну защитную заявку `STOP_LOSS`, действующую до отмены. Её направление зеркально стороне позиции, а идентификатор и фактическая цена хранятся в таблице `position_protections`. `TAKE_PROFIT` исполняется только запущенным ботом по цене `LastPrice`.

Чтобы не отдавать прибыль после удачного входа, бот использует те же `takeProfitPercent` и ATR для динамической защиты. При половине заданной цели прибыли он переводит уровень в безубыток с учётом оценочных комиссий, затем подтягивает его на расстояние одного ATR только в прибыльную сторону: ниже цены для LONG и выше цены для SHORT. Достигнутый уровень хранится как `managedExitPrice` со стадией `BREAKEVEN` или `TRAILING` и закрывается рыночной заявкой. Брокерский SL при этом **не заменяется**: отмена и повторное создание двух стоп-заявок неатомарны и могут оставить позицию без защиты либо создать риск переворота позиции. Поэтому при остановленном боте остаётся действовать первоначальный брокерский SL, а не динамически накопленная защита прибыли.

Поля `brokerStopLossPrice`, `managedExitPrice` и `profitProtectionStage` возвращаются nullable в `/positions` и `/dashboard`. Они показывают фактически подтверждённый брокерский уровень и бот-управляемую защиту; для исторической позиции, созданной до миграции, значение может отсутствовать.

`BrokerProtectionReconciliationService` выполняет пакетную сверку при старте приложения, после торгового сигнала и при запущенном боте использует пользовательский поток сделок T-Invest как основной сигнал. При исполнении направления, закрывающего текущую сторону позиции (SELL для LONG, BUY для SHORT), а также при ошибке/переподключении стрима повторно выполняются один запрос за портфелем и один — за статусами stop-orders. Резервная сверка запускается только при открытых позициях с интервалом `BROKER_PROTECTION_RECONCILIATION_DELAY_MS` (по умолчанию 3 минуты). После таймаута открытия событие остаётся `PENDING`, пока отмена не подтверждена или сверка не обнаружит фактическое исполнение; подтверждённый поздний `FILL` восстанавливается как обычная позиция бота с защитой. Внешняя позиция брокера без подходящего `OPEN`-события не принимается ботом в сопровождение. Для старой записи без сохранённой цены SL эта же сверка считывает цену уже активной заявки из ответа брокера и записывает её локально, не отменяя и не изменяя заявку. При обнаружении исполненного SL бот сохраняет `CLOSE` с причиной `STOP_LOSS`, фактическими ценой и комиссией дочерней биржевой заявки, идентификаторами stop- и биржевой заявок и числом исполненных лотов; затем рассчитывает P&L, убирает позицию из памяти и публикует обновления клиенту. Если локальная позиция отсутствует у брокера или сторона брокера противоположна, бот сохраняет `CLOSE` с причиной `BROKER_POSITION_RECONCILIATION`, без вымышленного P&L, и прекращает её сопровождение. Защитная заявка, не связанная с позицией в памяти, не отменяется автоматически: она может защищать фактическую внешнюю позицию. Если шорт остаётся открытым при недостаточной марже, он аварийно закрывается с причиной `MARGIN_RISK_CLOSE`.
...
### Выгрузка свечей для диагностики

`GET /internal/diagnostics/candles.csv?instrumentUid=<UUID>&from=2026-09-14T00:00:00Z&to=2026-09-15T00:00:00Z`

`POST /internal/diagnostics/candles/batch.csv`

```json
{
  "instrumentUids": ["<UUID-1>", "<UUID-2>"],
  "from": "2026-09-14T00:00:00Z",
  "to": "2026-09-15T00:00:00Z"
}
```

Отдельный внутренний endpoint для оператора использует тот же Basic Auth
(`INTERNAL_API_USERNAME`, `INTERNAL_API_PASSWORD`). Обращаться по HTTPS непосредственно
к боту; gateway и Android этот endpoint не используют. После установки обновления
повторные выгрузки не требуют перезапуска бота.

Первый endpoint экспортирует M5 для одного UID. Пакетный принимает JSON-массив до 50
уникальных UID и возвращает один CSV, отсортированный по UID и времени. Оба используют
один диапазон `[from, to)` до 7 суток,
`from < to`, конец не в будущем. Передавайте ISO-8601 с часовым поясом, предпочтительно
UTC (`Z`). Данные запрашиваются посуточно, сортируются и очищаются от дубликатов
по времени начала свечи. CSV UTF-8 содержит
`instrument_uid,interval,time_utc,open,high,low,close,volume,is_complete`.
Незавершённые свечи отмечены `false`; для анализа закрытых свечей исключайте их.
Пустая история возвращает заголовок без строк и сама по себе не означает сбой.
Предысторию для прогрева индикаторов нужно включить в запрошенный диапазон.

Одновременно допускается одна выгрузка на экземпляр приложения (иначе HTTP 429).
Ошибочные параметры дают HTTP 400, ошибка брокера — HTTP 502 без частичного CSV:
для пакетного экспорта сбой одного UID отменяет весь файл.
История запрашивается независимо от торгового кеша, не изменяет заявки, позиции
или стратегии и не требует запуска торгового цикла. Экспорт использует общую
квоту API брокера, поэтому не вызывайте его в цикле. Basic Auth обязателен;
секреты передавайте в заголовке авторизации, не в URL.
...
| `AI_PROVIDER` | AI-провайдер: `gemini` (по умолчанию) или `gigachat` |
| `GEMINI_API_KEY` | секретный ключ проекта Google AI Studio; нужен при `AI_PROVIDER=gemini` |
| `GEMINI_MODEL` | модель Gemini; по умолчанию `gemini-2.5-flash-lite` |
| `GEMINI_TIMEOUT_MS` | таймаут запроса к модели |
| `GIGACHAT_API_KEY` | ключ авторизации GigaChat; нужен при `AI_PROVIDER=gigachat`, не является access token |
| `GIGACHAT_MODEL` | модель GigaChat; по умолчанию `GigaChat-2` |
| `GIGACHAT_TIMEOUT_MS` | общий таймаут OAuth и запроса GigaChat; по умолчанию `5000` мс |
| `GIGACHAT_SCOPE` | OAuth scope; для личного ключа `GIGACHAT_API_PERS` |
